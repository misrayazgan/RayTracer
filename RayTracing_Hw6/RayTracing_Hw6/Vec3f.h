...
public:
	...
	Vec3f unitVector();
	int getAbsMinElementIndex() const;
	float getAvg() const;
	Vec3f pow(float exp);
...
inline int Vec3f::getAbsMinElementIndex() const
{
	int minIdx = 0;
	float min = abs((*this)[0]);
	for (int i = 1; i < 3; i++)
	{
		if (abs((*this)[i]) < min)
		{
...
inline float Vec3f::getAvg() const
{
	return (x + y + z) / 3.0f;
}

inline Vec3f Vec3f::pow(float exp)
{
	Vec3f res = Vec3f();
	res.x = powf(x, exp);
	res.y = powf(y, exp);
