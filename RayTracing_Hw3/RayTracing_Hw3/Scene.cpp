...
void Scene::loadSceneFromXml(const std::string& filepath)
{
    ...
    while (element)
    {
		...
		if (!plyFile)
		{
			...
			while (!(stream >> v0).eof())
			{
				...
				Triangle *triangle = new Triangle(this, v0, v1, v2, materialId, shadingMode, Matrix4f(1.0f), false, Vec3f(0.0f, 0.0f, 0.0f), false);
		...
		{
			parsePlyFile(filepath, plyFile, triangles, shadingMode, materialId);
		}
		...
		Mesh* baseMesh = new Mesh(this, materialId, triangles, transformationMat, transform, motionVec, motionBlur);
	...
	while (element)
	{
		...
		objects.push_back(new MeshInstance(this, materialId, baseMesh->bvh, transformationMat, transform, motionVec, motionBlur));
    ...
    while (element)
    {
		...
		objects.push_back(new Triangle(this, v0, v1, v2, materialId, SHADINGMODE_FLAT, transformationMat, transform, motionVec, motionBlur));
    ...
    while (element)
    {
		...
		objects.push_back(new Sphere(this, centerVertexId, radius, materialId, transformationMat, transform, motionVec, motionBlur));
...
void Scene::parsePlyFile(const std::string& filepath, const std::string& plyFile, std::vector<Object*>& triangles, ShadingMode shadingMode, int materialId)
{
	...
	for (int i = 0; i < faceIndices.size(); i++)
	{
		...
		else if (faceIndices[i].size() == 4)
		{
			...
			Triangle *triangle1 = new Triangle(this, v0, v1, v2, materialId, shadingMode, Matrix4f(1.0f), false, Vec3f(0.0f, 0.0f, 0.0f), false);
			Triangle *triangle2 = new Triangle(this, v0, v2, v3, materialId, shadingMode, Matrix4f(1.0f), false, Vec3f(0.0f, 0.0f, 0.0f), false);
...
void Scene::renderScenePartial(int minHeight, int maxHeight, int cameraIdx, unsigned char* image, std::default_random_engine& randGenerator, std::uniform_real_distribution<float>& distribution)
{
	...
	if (camera.numberOfSamples == 1)
	{
		for (int i = minHeight; i < maxHeight; i++)
		{
			...
			{
				Vec3f color = renderPixel(camera, i, j, randGenerator, distribution);
	...
	else
	{
		...
		for (int i = minHeight; i < maxHeight; i++)
		{
			for (int j = 0; j < width; j++)
			{
				Vec3f color = renderPixelMultisampling(camera, i, j, randGenerator, distribution);
...
Vec3f Scene::renderPixelMultisampling(const Camera& camera, int i, int j, std::default_random_engine& randGenerator, std::uniform_real_distribution<float>& distribution)
{
	...
	for (int r = 0; r < raysPerPixel.size(); r++)
	{
		...
		color += findPixelColor(ray, camera, maxRecursionDepth, randGenerator, distribution);
	}
...
Vec3f Scene::renderPixel(const Camera& camera, int i, int j, std::default_random_engine& randGenerator, std::uniform_real_distribution<float>& distribution)
{
	...
	Vec3f color = findPixelColor(ray, camera, maxRecursionDepth, randGenerator, distribution);
...
Ray Scene::generateRay(const Camera& camera, int i, int j, float time, float dx, float dy)
{
	...
	// j -> index along width
	float su = (camera.right - camera.left)*(j + dx)/(float)camera.imageWidth;
	float sv = (camera.top - camera.bottom)*(i + dy)/(float)camera.imageHeight;
...
Vec3f Scene::findPixelColor(const Ray& ray, const Camera& camera, int depth, std::default_random_engine& randGenerator, std::uniform_real_distribution<float>& distribution)
{
	Vec3f color = Vec3f();
	...
	if (result == true)
	{
		...
		if (ray.isInsideObject == false)
		{
			...
			for (int i = 0; i < areaLights.size(); i++)
			{
				...
				if (shadow == false)
				{
					Vec3f diffuse = diffuseShadingArea(areaLight, pointOnLight, hitResult, material);
		...
		if (material.type == MATERIALTYPE_MIRROR && depth > 0)
		{
			...
			color += material.mirror * findPixelColor(mirrorRay, camera, depth - 1, randGenerator, distribution);			
		}
		...
		if (material.type == MATERIALTYPE_DIELECTRIC && depth > 0)
		{
			...
			if (fr == 1.0f)
			{
				...
				Vec3f reflectionColor = findPixelColor(reflectionRay, camera, depth - 1, randGenerator, distribution);				
			...
			else
			{
				...
				Vec3f refractionColor = findPixelColor(refractionRay, camera, depth - 1, randGenerator, distribution);
...
{
	Vec3f diffuse = Vec3f();
	if (material.diffuseExists == true)
	{
...
Vec3f Scene::diffuseShadingArea(const AreaLight& areaLight, const Vec3f& point, const Hit& hit, const Material& material)
{
	Vec3f diffuse = Vec3f();
	if (material.diffuseExists == true)
	{
