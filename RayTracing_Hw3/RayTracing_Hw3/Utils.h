...
void printTimeDuration(T start, T end)
{
	...
	if (printedEarlier || mins >= 1) {
		printedEarlier = true;
		std::cout << mins << (1 != mins ? " minutes" : " minute") << ' ';
	}

	if (printedEarlier || secs >= 1) {
		printedEarlier = true;
		std::cout << secs << (1 != secs ? " seconds" : " second") << ' ';
	}

	if (printedEarlier || msecs >= 1) {
		printedEarlier = true;
		std::cout << msecs << (1 != msecs ? " milliseconds" : " millisecond") << ' ';
	}

	if (printedEarlier || usecs >= 1) {
		printedEarlier = true;
		std::cout << usecs << (1 != usecs ? " microseconds" : " microsecond");
	}
}
